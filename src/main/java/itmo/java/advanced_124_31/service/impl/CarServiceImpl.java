...
public class CarServiceImpl implements CarService {
	...
	public CarDTORequest update(Long id, CarDTORequest carDTORequest) {
		...
		carRepository.findById(id).ifPresentOrElse(c -> {
			if (carDTORequest.getStateNumber() != null && !carDTORequest.getStateNumber()
					.isEmpty()) {
				checkStateNumber(carDTORequest.getStateNumber());
			}
	...
	public ModelMap getCars(Integer page, Integer perPage, String sort,
		...
		List<CarDTORequest> content = pageResult.getContent().stream().map(
				c -> mapper.convertValue(c, CarDTORequest.class)).collect(
				Collectors.toList());
	...
	/**
	 * Returns a {@link CarDTORequest} object after cleaning the connection between Car
	 * and its Driver
	 *
	...
	/**
	 * Change the state of {@link Car}-entity by chosen and set the entity field updatedAt
	 * new local date time
	 *
